...
config_file = json.load(open("scripts/config/config_nq.json", "r"))
...
model_name = "DDRO" # / DDRO /SFT
dataset = "nq" # msmarco / nq

def main():
    # for epoch in [1,3,5,7,9]:
    epoch = 9
    os.system(f"cd {code_dir}/utils && python run_t5_trainer.py \
        --epoch 10 \
        --per_gpu_batch_size 4 \
        --save_path {code_dir}/outputs-nq/t5_128_1_pq_pretrain_search_finetune/model_final.pkl \
        --log_path {code_dir}/logs-nq/{model_name}-{dataset}-{encoding}.log \
