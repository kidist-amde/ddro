...
def evaluate_beamsearch():
    """
    Performs evaluation with constrained beam search and evaluates ranking results.
    """
    # Load the tokenizer and set vocabulary size based on it
    tokenizer = T5Tokenizer.from_pretrained(args.pretrain_model_path)
    vocab_size = len(tokenizer)

    # Load the pretrain model and resize to match the tokenizer's vocabulary size
    pretrain_model = T5ForConditionalGeneration.from_pretrained(args.pretrain_model_path)
    pretrain_model.resize_token_embeddings(vocab_size + args.add_doc_num)  # Match tokenizer's vocab size

    # Initialize T5ForPretrain with the resized model
    model = T5ForPretrain(pretrain_model, args)

    # Load the checkpoint
    save_model = torch.load(args.save_path, map_location=device)

    # Filter the state dictionary to only include compatible keys
    model_dict = model.state_dict()
    filtered_checkpoint = {k: v for k, v in save_model.items() if k in model_dict and model_dict[k].shape == v.shape}

    # Load the filtered state dictionary
    model.load_state_dict(filtered_checkpoint, strict=False)  # Allow partial loading with filtered keys
    model = model.to(device)
    model.eval()
    
    # Rest of evaluate_beamsearch() code remains the same
        ...
        for i, testing_data in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
            ...
            for j in range(input_ids.shape[0]):
                ...
                for docid in batch_output:
                    ...
                    if docid_str not in encode_2_docid:
                        continue

                    if args.use_docid_rank == "False":
                        doc_rank.append(docid_str)
                    else:
                        docid_list = encode_2_docid[docid_str]
                        if len(docid_list) > 1:
