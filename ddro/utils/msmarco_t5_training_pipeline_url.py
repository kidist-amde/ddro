...
# # because of time lilimt I resturt the training from epoch 11 
# # --load_ckpt_path {code_dir}/outputs-msmarco/t5_128_10_top_300k_url_title_pretrain_search_ULTRON/model_11.pkl \

# os.system(f"cd {code_dir}/utils && python runT5.py \
#     --epoch {epoch} \
#     --per_gpu_batch_size  100 \
#     --learning_rate 1e-3 \
#     --save_path {code_dir}/outputs-msmarco/{model}_{top_or_rand}_{scale}_{encoding}_{all_data}_ULTRON_Resumefrom_11/ \
#     --log_path {code_dir}/logs-msmarco/{stage}.{model}.{top_or_rand}.{scale}.{encoding}.{all_data}.log \
#     --doc_file_path {code_dir}/resources/datasets/processed/msmarco-data/msmarco-docs-sents.{top_or_rand}.{scale}.json \
#     --pretrain_model_path {code_dir}/resources/transformer_models/t5-base \
#     --docid_path {code_dir}/resources/datasets/processed/msmarco-data/encoded_docid/t5_512_url_{top_or_rand}.{scale}.txt \
...
os.system(f"cd {code_dir}/utils && python runT5.py \
    ...
    --use_origin_head {use_origin_head} \
    --load_ckpt {load_ckpt} \
    --load_ckpt_path {code_dir}/outputs-msmarco/{load_model}_{top_or_rand}_{scale}_{encoding}_pretrain_search_ULTRON_Resumefrom_11/model_final.pkl \
    --output_every_n_step 5000 \
    --save_every_n_epoch 2 \
    --operation {operation}")
