...
os.system(f"cd {code_dir}/utils && python runT5.py \
    ...
    --learning_rate 1e-3 \
    --save_path {code_dir}/outputs-summaries-msmarco/{model}_{top_or_rand}_{scale}_{encoding}_{all_data}/ \
    --log_path {code_dir}/logs-summaries/{stage}.{model}.{top_or_rand}.{scale}.{encoding}.{all_data}.log \
...
os.system(f"cd {code_dir}/utils && python runT5.py \
    ...
    --use_origin_head {use_origin_head} \
    --load_ckpt {load_ckpt} \
    --load_ckpt_path {code_dir}/outputs-summaries-msmarco/{load_model}_{top_or_rand}_{scale}_{encoding}_pretrain/model_final.pkl \
    --output_every_n_step 5000 \
...
os.system(f"cd {code_dir}/utils && python runT5.py \
    ...
    --use_origin_head {use_origin_head} \
    --load_ckpt {load_ckpt} \
    --load_ckpt_path {code_dir}/outputs-summaries-msmarco/{load_model}_{top_or_rand}_{scale}_{encoding}_pretrain_search/model_final.pkl \
    --output_every_n_step 5000 \
    --save_every_n_epoch 2 \
    --operation {operation}")
