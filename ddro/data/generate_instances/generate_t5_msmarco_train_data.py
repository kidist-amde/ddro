...
def gen_fake_query_instance(id_to_token, token_to_id, all_docid, encoded_docid):
    ...
    try:
        ...
        print(f"Error: `fake_query_path` file {args.fake_query_path} not found.")
    except IOError as e:
        print(f"IOError while writing to {output_path}: {e}")
    
    # Check if the output file was created
    if not os.path.exists(output_path):
        print(f"Error: Expected output file {output_path} was not created.")
    else:
        print(f"Fake query data generation completed successfully at {output_path}")
