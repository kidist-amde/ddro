...
class Csv(datasets.ArrowBasedBuilder):
    ...
    def _generate_tables(self, files):
        """Generate PyArrow tables from CSV files in chunks."""
        schema = pa.schema(self.config.features.type) if self.config.features else None
        ...
        for file_idx, file in enumerate(files):
            ...
            for batch_idx, df in enumerate(csv_file_reader):
                ...
                logger.info(f"Processed batch {batch_idx} from file {file} with {pa_table.num_rows} rows.")
                
                # Yield the table for further processing
